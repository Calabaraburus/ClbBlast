      ...
      _export("FieldModel", FieldModel = (_dec = ccclass('FieldModel'), _dec2 = property({
        ...
        */
        ...
        getTileModelByMapMnemonic(mnemonic) {
          var map = this.mnemMapping.filter(item => item.mnemonic == mnemonic);
          ...
          Button;
          var res = this.tiles.filter(item => item.tileName == map[0].tileName);
          return res.length != 0 ? res[0] : this.tiles[0];
        }

        getFieldMap() {
          var result = [];
          var textLines = this.fieldMap.text.split(/\r?\n/);
          textLines.forEach((line, i) => {
            var iinv = textLines.length - i - 1;
            result[iinv] = [];
...
//# sourceMappingURL=96667992b6e804f6b410424822ab49a5df34b258.js.map