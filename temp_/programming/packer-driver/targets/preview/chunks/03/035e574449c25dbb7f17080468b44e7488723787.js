    ...
    execute: function () {
      ...
      _export("FieldController", FieldController = (_dec = ccclass('FieldController'), _dec2 = property({
        ...
        generateTiles() {
          ...
          this._field = [];
          var map = this.fieldModel.getFieldMap();

          for (var yIndex = 0; yIndex < this.fieldModel.rows; yIndex++) {
            this._field[yIndex] = [];

            for (var xIndex = 0; xIndex < this.fieldModel.rows; xIndex++) {
              var tileModel = this.fieldModel.getTileModelByMapMnemonic(map[yIndex][xIndex]);
        ...
        createTile(_ref) {
          ...
          var tile = this.tileCreator.create(tileModel.tileName);
          var tileController = tile.getComponent(_crd && TileController === void 0 ? (_reportPossibleCrUseOfTileController({
            error: Error()
          ...
          var size = this.calculateTileSize(tile);
          tile.scale = size;
        ...
        calculateTilePosition(row, col) {
          var border = this.fieldModel.border / 2;
          var tW = this.tilesArea.width / this.fieldModel.cols;
        ...
        calculateTileSize(tile) {
          ...
          var tW = this.tilesArea.width / this.fieldModel.cols;
          var coef = tW / (tileTransform.width + this.fieldModel.border);
        ...
        moveAllTilesOnARote(roteId) {
          var startTile = this.getStartTile(roteId);
          var endTile = this.getEndTile(roteId);
          var emptyModel = this.fieldModel.getTileModel("empty");

          if (startTile == null || endTile == null) {
            return;
          }

          var findTiles = destroied => {
            var res = [];
          ...
          var fwd = endTile.row > startTile.row;
          var destroiedTiles = findTiles(true);

          if (destroiedTiles.length == 0) {
            return;
          }

          var stdTileModels = this.fieldModel.getStandartTiles();
          var pathTiles = []; // add new tiles

          for (var index = 0; index < destroiedTiles.length; index++) {
            var tileRowId = fwd ? startTile.row + 1 + index : startTile.row - 1 - index;
            var yPosIndex = fwd ? startTile.row - 1 - index : startTile.row + 1 + index;
            var tile = this.createTile({
              row: tileRowId,
              col: roteId,
          ...
          pathTiles.forEach((t, i) => {
            var tileRowId = fwd ? startTile.row + 1 + i : startTile.row - 1 - i;
            t.row = tileRowId;
        ...
        getStartTile(roteId) {
          var startModel = this.fieldModel.getTileModel('start');
          return this.getTile(roteId, startModel);
        }

        getEndTile(roteId) {
          var startModel = this.fieldModel.getTileModel('end');
          return this.getTile(roteId, startModel);
        }
        ...
        update(deltaTime) {
          if (this._timeToexecute < 0 && this._canexecute) {
            ...
            for (var index = 0; index < this.fieldModel.cols; index++) {
...
//# sourceMappingURL=035e574449c25dbb7f17080468b44e7488723787.js.map